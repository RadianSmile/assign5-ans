...
console.log ('https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22'+cityname+'%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys');
...
$('#dropdown li').each(function(index , element){
  var city = ($(element).attr("id")) ;

  $.getJSON(
      'https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D"' + city + '")&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys',function(data,status){

      $("<span>",{
        text : Math.round(tempftoc(data.query.results.channel.item.condition.temp)) + "°",
        class: "pull-right"
      }).appendTo($(element).find("a"));
  });
})
